 ...
 * @author Anderson
 */
public class funcionApt4 extends FitnessFunction {
   private double fitness;
    ...
    private void Evaluar(IChromosome cromosoma) {
        ...
        valorY = (274 - Math.abs( (((Math.pow(x, 2)-(4*x)+7))*Math.log10(5)) + Math.log10(16)-4))-274;
        System.out.println(x +" ; "+ valorY );
       
        fitness = 274 - ( (((Math.pow(x, 2)-(4*x)+7))*Math.log10(5)) + Math.log10(16)-4);
        //(x^2-4x+7)lg5+lg16 = 4
    }

    public double getY(float x) {
        double v_opt_y=0;
        v_opt_y = (274 - Math.abs((((Math.pow(x, 2)-(4*x)+7))*Math.log10(5)) + Math.log10(16)-4))-274;
        return v_opt_y;
    }
