 ...
 * @author Anderson
 */
public class funcionApt1 extends FitnessFunction{
    ...
    private void Evaluar(IChromosome cromosoma) {
        ...
        valorY = (110 - (((Math.pow(x, 2)-(5*x)+9) * Math.log10(2)) + Math.log10(125) - 3 ))-110;
        System.out.println(x +" ; "+ valorY );
        //fitness = 256 - Math.abs(valor - 50);
        fitness = 110 - ( ((Math.pow(x, 2)-(5*x)+9) * Math.log10(2)) + Math.log10(125) - 3 );
        //(x^2-5x+9) lg2 + lg125 = 3
    }

    public double getY(float x) {
        double v_opt_y=0;
        v_opt_y = (110 - (((Math.pow(x, 2)-(5*x)+9) * Math.log10(2)) + Math.log10(125) - 3 ))-110;
        return v_opt_y;
    }
